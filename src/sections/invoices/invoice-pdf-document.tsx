...
export const InvoicePdfDocument: FC<InvoicePdfDocumentProps> = (props) => {
  ...
  const dueDate = invoice.due_on && format(Date.parse(invoice.due_on), 'dd MMM yyyy');
  const issueDate = invoice.issued_on && format(Date.parse(invoice.issued_on), 'dd MMM yyyy');
